...
const ColorRGBA GLVolume::NO_TRAVEL_COLOR          = { 1.0f, 0.45f, 0.1f, 0.5f }; // orange
    ...
    if (force_native_color || force_neutral_color) {
        ...
        if (hover == HS_Deselect)
            ...
            if (color == NEGATIVE_VOLUME_COLOR || color == PARAMETER_MODIFIER_COLOR ||
                ...
                color == SEAM_POSITION_COLOR || color == SEAM_POSITION_INTERNAL_COLOR || color == BRIM_PATCH_COLOR ||
                color == BRIM_NEGATIVE_COLOR || color == NO_TRAVEL_COLOR)
...
ColorRGBA color_from_model_volume(const ModelVolume& model_volume)
{
    ...
    else if (model_volume.is_no_travel())
        color = GLVolume::NO_TRAVEL_COLOR;
