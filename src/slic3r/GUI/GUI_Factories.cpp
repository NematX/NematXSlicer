...
static const constexpr std::array<std::pair<const char *, const char *>, 11> ADD_VOLUME_MENU_ITEMS = {{
    ...
    {L("Add internal seam attractor (cylinder)"), "add_seam"},            // ~ModelVolumeType::SEAM_POSITION_INTERNAL_CENTER_Z
...
void MenuFactory::append_submenu_add_generic(wxMenu* menu_parent, wxMenu* sub_menu, ModelVolumeType type)
{
    ...
    assert(type != ModelVolumeType::SEAM_POSITION_INTERNAL_CENTER_Z);
...
void MenuFactory::append_menu_items_add_volume(MenuType menu_type)
{
    ...
    if (menu_type != mtObjectSLA) {
        ...
        append_menu_item(sub_menu_both, wxID_ANY, _L("Seam (internal perimeter) cylinder attractor"), "",
                [this](wxCommandEvent&) { obj_list()->load_generic_subobject(L("SmallCylinder"), ModelVolumeType::SEAM_POSITION_INTERNAL_CENTER_Z); },
                item_cylinder.second, nullptr, selected_func, m_parent);
        // seam inside
