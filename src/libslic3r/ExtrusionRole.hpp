...
public:
    ...
    ExtrusionRole(ExtrusionRoleModifier mods) : m(mods) {
        ...
        // check only one base type (pow 2) (unless it's travel)
        assert( ((m & ExtrusionRole::BaseType) & ((m & ExtrusionRole::BaseType) - 1)) == 0 || (mods & ExtrusionRoleModifier::ERM_Travel) == ExtrusionRoleModifier::ERM_Travel);
    }
...
enum class GCodeExtrusionRole : uint8_t {
    ...
    // for post-processing
    Travel,
    // special kind of travel
    RectilinearAroundHoleInfillTravel,
