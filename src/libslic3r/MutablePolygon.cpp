...
void remove_duplicates(MutablePolygon &polygon, coord_t scaled_eps, const double max_angle)
{
    if (polygon.size() >= 3) {
        ...
        for (++it; it != begin;) {
            ...
            distsqrf_t v1_sqr_norm = squared_norm(v1);
...
static bool clip_narrow_corner(
    ...
    distsqrf_t                  dist2_current, 
    const distf_t               shortcut_length)
{
    MutablePolygon &polygon = it0.polygon();
    assert(polygon.size() >= 2);

    const distsqrf_t shortcut_length2 = sqr(shortcut_length);
    ...
    distsqrf_t dist2_next = 0;

    assert(p0.distance_to(p2) < shortcut_length);
    ...
    while (polygon.size() >= 3) {
        ...
        if (forward == Far && backward == Far) {
            ...
            p02 = *it0.prev();
            p22 = *it2.next();
            //auto/*int64_t*/ d2 = (p22 - p02).squaredNorm();
            distsqrf_t d2 = p22.distance_to_square(p02);
        ...
        if (forward == Free && (backward != Free || p2.distance_to_square(p1) < p0.distance_to_square(p1))) {
            ...
            if (cross2_double(p2 - p0, p22 - p0) > 0)
                forward = Blocked;
            else {
                // New clipping edge lenght.
                //auto d2 = (p22 - p0).squaredNorm();
                distsqrf_t d2 = p22.distance_to_square(p0);
                if (d2 > shortcut_length2) {
                    forward    = Far;
                    dist2_next = d2;
                } else {
            ...
            p02 = *it0.prev();
            if (cross2_double(p02 - p2, p0 - p2) > 0)
                backward = Blocked;
            else {
                // New clipping edge lenght.
                //auto d2 = (p2 - p02).squaredNorm();
                distsqrf_t d2 = p2.distance_to_square(p02);
                if (d2 > shortcut_length2) {
                    backward   = Far;
                    dist2_next = d2;
                } else {
    ...
    if (polygon.size() <= 3) {
        ...
        if (polygon.size() == 3) {
            ...
            auto orient1 = cross2_double(p02 - p2, p0 - p2);
            auto orient2 = cross2_double(p2 - p0, p22 - p0);
    ...
    if ((forward == Blocked && backward == Blocked) || dist2_current > sqr(shortcut_length - distf_t(SCALED_EPSILON))) {
        // The crack is filled, keep the last clipping edge.
    //} else if (dist2_next < sqr(shortcut_length - int64_t(SCALED_EPSILON))) {
    } else if (dist2_next < sqr(shortcut_length - distf_t(SCALED_EPSILON))) {
        ...
        const coordf_t  a = v.squaredNorm();
...
void smooth_outward(MutablePolygon &polygon, coord_t clip_dist_scaled)
{
    ...
    while (! unprocessed_range.empty() && polygon.size() > 2) {
        ...
        Vec2d v1d   = (p0 - p1).cast<double>();
        Vec2d v2d   = (p2 - p1).cast<double>();
        //if (cross2(v1, v2) > 0) {
        if (cross2(v1d, v2d) > 0) {
            // Concave corner.
            double dot  = v1d.dot(v2d);
            //auto    l2v1 = double(v1.squaredNorm());
            //auto    l2v2 = double(v2.squaredNorm());
            distsqrf_t  l2v1 = v1d.squaredNorm();
            distsqrf_t  l2v2 = v2d.squaredNorm();
            if (dot > 0 || Slic3r::sqr(double(dot)) * 2. < l2v1 * l2v2) {
                ...
                Vec2crd v02  = (p2 - p0);
                distsqrf_t l2v02 = squared_norm(v02);
                it1.remove();
                if (l2v02 < clip_dist_scaled2) {
                    ...
                    if (clip_narrow_corner(p1, it0, it2, unprocessed_range, l2v02, distf_t(clip_dist_scaled)))
                    ...
                    if (l2v02 > clip_dist_scaled2eps) {
                        ...
                        if (d2 < foot_dist_min2) {
                            ...
                            const double    b = - 2. * v1d.dot(v2d);
                            double          u = b * b - 4. * l2v2 * (l2v1 - clip_dist_scaled2);
                            assert(u > 0.);
    ...
    if (polygon.size() == 3) {
        ...
        if (cross2_double(v1, v2) > 0) {
            ...
            const Vec2crd v3 = (p2 - p0);
            distsqrf_t l12 = squared_norm(v1);
            distsqrf_t l22 = squared_norm(v2);
            distsqrf_t l32 = squared_norm(v3);
            if (l22 > l12 && l22 > l32) {
                std::swap(v1,  v2);
                std::swap(l12, l22);
            ...
            double h2 = l22 - sqr(double(v1.dot(v2))) / double(l12);
