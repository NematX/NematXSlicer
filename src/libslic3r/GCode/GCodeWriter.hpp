...
public:
    ...
    static std::string set_fan(const GCodeFlavor gcode_flavor, bool gcode_comments, uint8_t speed, uint8_t tool_fan_offset,
        uint16_t tool_id, bool is_fan_percentage, const std::string_view comment = {});
    // To be called by the main thread. It always emits the G-code, it does remember the previous state
    //   to be able to reset after the wipe tower (but remove that when the wipe tower will be extrusions and not string).
