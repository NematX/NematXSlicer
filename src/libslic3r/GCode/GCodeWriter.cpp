...
void GCodeWriter::_extrude_e(GCodeFormatter &w, double dE)
{
    ...
    if (is_extrude) {
        ...
        if((delta < 0.00000001) & (delta > -0.00000001)) delta = 0;
...
std::string GCodeWriter::set_fan(const GCodeFlavor gcode_flavor, bool gcode_comments, uint8_t speed, uint8_t tool_fan_offset, uint16_t tool_id, bool is_fan_percentage, const std::string_view comment/*=""*/)
{
/*
    std::ostringstream gcode;
    if (speed == 0) {
