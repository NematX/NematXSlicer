...
public:
    ...
    bool operator< (const LayerTools &rhs) const { return _print_z < rhs._print_z; }
    bool operator==(const LayerTools &rhs) const { return _print_z == rhs._print_z; }
    ...
    coord_t                     wipe_tower_layer_height = 0.;
...
private:
    ...
    LayerTools(const coord_t z, bool scaledok) : _print_z(z) {}
...
public:
    ...
    const LayerTools*	tools_for_layer(coord_t print_z) const;
    LayerTools*			tools_for_layer(coord_t print_z) { return const_cast<LayerTools*>(std::as_const(*this).tools_for_layer(print_z)); }
...
private:
    ...
    void fill_wipe_tower_partitions(const PrintConfig &config,
                                    coord_t object_bottom_z,
                                    coord_t max_layer_height);
    bool                insert_wipe_tower_extruder();
    void                mark_skirt_layers(const PrintConfig &config, coord_t max_layer_height);
    ...
    const PrintObjectConfig*    m_print_object_config_ptr = nullptr;
};
