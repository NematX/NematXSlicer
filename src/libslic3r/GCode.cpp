...
void GCodeGenerator::GCodeOutputStream::write(const char *what)
{
    if (what != nullptr) {
        ...
        // ensure line size limit
        for (size_t i = 0; i < gcode.size(); i++) {
            if (gcode[i] != '\n') {
                line_length++;
                if (line_length > 4000) {
                    // hard line return
                    fwrite(gcode.c_str(), 1, i, this->f);
                    gcode = "\n; " + gcode.substr(i);
                    line_length = 2;
                }
            } else {
...
std::string GCodeGenerator::_before_extrude(const ExtrusionPath &path, const std::string_view description_in, double speed_mm_s) {
    ...
    if (m_delayed_layer_change.empty()) {
        ...
        assert(is_approx(m_writer.get_position().z(), m_layer->print_z, EPSILON) || m_need_z_reset_after_path_3D || m_loop_vase_mode ||
...
void GCodeGenerator::write_travel_to(std::string &gcode, Polyline& travel, std::string comment)
{
    ...
    assert(m_need_z_reset_after_path_3D || is_approx(this->writer().get_unlifted_position().z(), m_layer->print_z, EPSILON) || m_loop_vase_mode || comment == "Travel to a Wipe Tower" ||
               (config().external_perimeters_first && config().external_perimeters_staggered));
}
